...
omnihost uptime tf-demo-vm        # Quick uptime
omnihost disk tf-demo-vm          # Disk usage (df -h)
omnihost memory tf-demo-vm        # Memory usage (free -h)
omnihost cpu tf-demo-vm           # CPU info
omnihost processes tf-demo-vm     # Top processes

# Service management
omnihost status tf-demo-vm nginx      # Service status
omnihost restart tf-demo-vm nginx     # Restart service
omnihost logs tf-demo-vm nginx -n 100 # View logs

# Custom log file
omnihost logs tf-demo-vm /var/log/syslog -n 50
```
...
omnihost exec-all "apt update" --no-output
```
...
# Database servers
omnihost exec-multi "db01,db02" "pg_isready"

# With parallel control
omnihost exec-multi "app01,app02,app03,app04" "git pull" --parallel 4
```
...
```bash
omnihost exec-all "uptime" --parallel 20
```

#### 2. Deploy application to web servers
```bash
omnihost exec-multi "web01,web02,web03" "cd /app && git pull && systemctl restart app" --parallel 3
```
...
```bash
omnihost exec-multi "app01,app02,app03" "systemctl is-active nginx"
```

#### 5. Quick health check
```bash
omnihost exec-all "uptime && free -h | grep Mem" --parallel 15
```

#### 6. Restart services across load balancer pool
```bash
omnihost exec-multi "lb01,lb02" "systemctl reload haproxy"
```

#### 7. Collect logs from multiple servers
```bash
omnihost exec-all "journalctl -u app -n 10 --no-pager" --show-output
```
...
omnihost exec-all "free -h" --parallel 10

# Disk usage on production servers
omnihost exec-multi "prod01,prod02,prod03" "df -h"

# Check specific service
omnihost exec-all "systemctl is-active docker" --parallel 20

# Find process
omnihost exec-all "ps aux | grep python" --no-output
```
...
omnihost exec-all "apt upgrade -y" --parallel 2

# High parallelism for quick checks
omnihost exec-all "uptime" --parallel 20

# Default is 5 - good for most cases
omnihost exec-all "systemctl status nginx"
```

### 2. Use quick commands for common tasks
```bash
# Instead of:
omnihost exec server01 "df -h"

# Use:
omnihost disk server01
```
...
omnihost exec-all "ps aux | wc -l"
```

### 4. Use timeout for slow commands
```bash
# Don't wait forever
omnihost exec-all "wget example.com" --timeout 5
```
...
omnihost exec-all "systemctl restart app" --continue

# Stop on first error
omnihost exec-all "systemctl restart app" --stop
```

### Output Control
```bash
# Show all outputs (default)
omnihost exec-all "uptime" --show-output
...
omnihost exec-all "apt upgrade -y" --timeout 300
```
...
omnihost exec-multi "web01,web02,web03" "systemctl restart nginx" --parallel 1
```

### Health Check All Servers
```bash
omnihost exec-all "curl -sf localhost:8080/health || echo UNHEALTHY"
```

### Update All Servers
```bash
omnihost exec-all "sudo apt update && sudo apt upgrade -y" --parallel 3 --timeout 300
```

### Gather Metrics
```bash
omnihost exec-all "uptime && free -h && df -h /" --show-output > metrics.txt
```

### Quick Incident Response
```bash
# Check all servers for suspicious activity
omnihost exec-all "last -n 20" --parallel 10

# Check for specific process
omnihost exec-all "ps aux | grep suspicious_process"
```
